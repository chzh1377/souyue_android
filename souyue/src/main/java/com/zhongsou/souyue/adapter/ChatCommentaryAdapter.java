...
public class ChatCommentaryAdapter extends BaseAdapter implements OnClickListener,IVolleyResponse {
...
//		http = new Http(this);
	...
	protected View genItemView(int position) {
		View v = inflaterVeiw(R.layout.chat_commentary_list_item);
	...
	private void setViewData(ViewHolder holder, final Comment comment) {
		if (comment != null && holder != null) {
		    String txt = null;
		    if(comment.replyTo() != null){
		        txt = comment.replyTo().user().name();
		    }
			...
			public void onClick(View arg0) {
				...
				Dialog dialog = builder.setTitle(report).setItems(itemnames, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						SrpReportReq report = new SrpReportReq(HttpCommon.SRP_CHAT_ROOM_REQUEST,ChatCommentaryAdapter.this);
