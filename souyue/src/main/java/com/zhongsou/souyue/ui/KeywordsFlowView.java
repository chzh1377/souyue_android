...
public class KeywordsFlowView extends FrameLayout implements OnGlobalLayoutListener {
     ...
     * 存储显示的关键字。
     */
    public Vector<SearchTop> vecKeywords;
    ...
    private OnSearchFlyClickListener onSearchFlyClickListener;
    ...
    private void init() {
        ...
        random = new Random();
        vecKeywords = new Vector<SearchTop>(MAX);
    ...
    public boolean feedKeyword(SearchTop keyword) {
    ...
    public interface OnSearchFlyClickListener {
        //点击textview执行的方法
        public void onTvClick(TextView tv);
    }

    public void setOnSearchFlyClickListener(OnSearchFlyClickListener listener) {
        this.onSearchFlyClickListener = listener;
    }
    ...
    private boolean show() {
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
            ...
            for (int i = 0; i < size; i++) {
                String title = vecKeywords.get(i).title();
                // 随机颜色  
                ...
                txt.setText(title);
                txt.setTag(R.id.tag_first, vecKeywords.get(i));
                ...
                txt.setOnClickListener(new OnClickListener() {
                    public void onClick(View v) {
                        onSearchFlyClickListener.onTvClick((TextView) v);
                    }
                });
//                txt.setBackgroundColor(Color.RED);
                // 获取文本长度  
                Paint paint = txt.getPaint();
                int strWidth = (int) Math.ceil(paint.measureText(title));
    ...
    public Vector<SearchTop> getKeywords() {
        return vecKeywords;
    }
