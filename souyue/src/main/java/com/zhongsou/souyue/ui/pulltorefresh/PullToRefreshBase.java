...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private boolean mForceLoading;

    private OnRefreshListener<T> mOnRefreshListener;
    private OnRefreshListener2<T> mOnRefreshListener2;
    private OnTimeRefreshListener mOnTimeRefreshListener;
    private OnCompleteRefreshListener mCompleteRefreshListener;
    ...
    public void setmForceLoading(boolean mForceLoading) {
        this.mForceLoading = mForceLoading;
    }

    public boolean ismForceLoading() {
        return mForceLoading;
    }
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        try {
            ...
            if (this.mOnTimeRefreshListener != null)
                mOnTimeRefreshListener.onTimeRefresh();
...
//        resetHeader();
    ...
    @Override
    public void onUpdateTime(String text) {
        if (null != mHeaderLayout){
            if(text == null){
                text = "刚刚";
                mHeaderLayout.setRefreshTime("最后刷新时间：" + text);
            }else {
                if ("".equals(text))
...
//        }
    ...
    public void setOncompleteListener(OnCompleteRefreshListener _completeRefreshListener){
        mCompleteRefreshListener = _completeRefreshListener;
    }
...
//        }
    ...
    public void setCanPullDown(boolean _pull){
        mCanPullDown = _pull;
    }
...
//            mFooterLayout.setLoadingDrawable(drawable);
    ...
    @Override
    public void setOnTimeRefreshListener(OnTimeRefreshListener listener) {
        mOnTimeRefreshListener = listener;
...
//    protected NewYearLoadingLayout createNewYearLoadingLayout(Context context, Mode mode, TypedArray attrs) {
//        return new NewYearLoadingLayout(context, mode, attrs);
//    }
    /**
    ...
    protected final AbstractLoadingLayout getHeaderLayout() {
...
//        }
        if (mSpaceTop!=0) {
            setHeaderScroll(mSpaceTop);
        }
        smoothScrollTo(0);
        postDelayed(new Runnable(){
            @Override
            public void run() {
                if(mCompleteRefreshListener!=null) {
                    mCompleteRefreshListener.onCompleteRefresh();
                }
            }
        },SMOOTH_SCROLL_DURATION_MS);
        if (this.mOnTimeRefreshListener != null)
...
//        ((ListView)mRefreshableView).setSelection(0);
    ...
    public void setSpaceTop(int _top){
        mSpaceTop = _top;
    }
...
//        boolean isFirst=attrs.getAttributeBooleanValue(xmlns, "isfirst", false);
//        if(isFirst){
//            mHeaderLayout= createNewYearLoadingLayout(context, Mode.PULL_DOWN_TO_REFRESH, a);
//        }else{
...
//        }
    ...
    private void refreshLoadingViewsHeight() {
        ...
        if(ConfigApi.isSouyue()){
            ...
            switch (mMode) {
                case DISABLED:
                    setPadding(0, 0-((ListView)mRefreshableView).getDividerHeight(), 0, 0);
                case BOTH:
                    setPadding(0, -mHeaderHeight-((ListView)mRefreshableView).getDividerHeight(), 0, -mHeaderHeight);
                    break;
                case PULL_UP_TO_REFRESH:
                    setPadding(0, 0-((ListView)mRefreshableView).getDividerHeight(), 0, -mHeaderHeight);
                    break;
                case PULL_DOWN_TO_REFRESH:
                default:
                    setPadding(0, -mHeaderHeight-((ListView)mRefreshableView).getDividerHeight(), 0, 0);
                    break;
        }
        }
    }
     ...
     * {@link com.zhongsou.souyue.ui.pulltorefresh.PullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
     *
     * @author Chris Banes
     */
    ...
    public static interface OnCompleteRefreshListener {
