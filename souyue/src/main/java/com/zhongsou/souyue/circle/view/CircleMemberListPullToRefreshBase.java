...
public abstract class CircleMemberListPullToRefreshBase<T extends View> extends LinearLayout {
	...
	private CircleMemberListLoadingLayout headerLayout;
	private CircleMemberListLoadingLayout footerLayout;
	...
	public CircleMemberListPullToRefreshBase(Context context, int mode) {
		super(context);
		this.mode = mode;
		init(context, null);
	}

	public CircleMemberListPullToRefreshBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}
	...
	protected final CircleMemberListLoadingLayout getFooterLayout() {
		return footerLayout;
	}

	protected final CircleMemberListLoadingLayout getHeaderLayout() {
		return headerLayout;
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshcircle);
		if (a.hasValue(R.styleable.PullToRefreshcircle_mode1)) {
			mode = a.getInteger(R.styleable.PullToRefreshcircle_mode1, MODE_PULL_DOWN_TO_REFRESH);
		}
		...
		String pullLabel = context.getString(R.string.circle_pull_to_refresh_pull_label);
		String refreshingLabel = context.getString(R.string.pull_to_refresh_refreshing_label);
		String releaseLabel = context.getString(R.string.circle_pull_to_refresh_release_label);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new CircleMemberListLoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new CircleMemberListLoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel, pullLabel, refreshingLabel);
		...
		// Styleables from XML
		if (a.hasValue(R.styleable.PullToRefreshcircle_headerTextColor)) {
			final int color = a.getColor(R.styleable.PullToRefreshcircle_headerTextColor, Color.parseColor("ff7e7e7e"));
			if (null != headerLayout) {
				headerLayout.setTextColor(color);
			}
			if (null != footerLayout) {
				footerLayout.setTextColor(Color.parseColor("ff7e7e7e"));
			}
		}
		if (a.hasValue(R.styleable.PullToRefreshcircle_headerBackground)) {
			this.setBackgroundResource(a.getResourceId(R.styleable.PullToRefreshcircle_headerBackground, Color.WHITE));
		}
		if (a.hasValue(R.styleable.PullToRefreshcircle_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(R.styleable.PullToRefreshcircle_adapterViewBackground,
					Color.WHITE));
		}
		a.recycle();
