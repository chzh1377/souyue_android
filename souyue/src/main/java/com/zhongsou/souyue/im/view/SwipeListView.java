...
public class SwipeListView extends ListView {
    ...
    public SwipeListView(Context context, AttributeSet attrs, int defStyle) {
      ...
      //获取自定义属性和默认值  
      mRightViewWidth = (int) mTypedArray.getDimension(R.styleable.swipelistviewstyle_right_width, 338);
    ...
    boolean isCurrentChildEnable = false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mIsHorizontal = null;
                isCurrentChildEnable = false;
                System.out.println("onInterceptTouchEvent----->ACTION_DOWN");
                mFirstX = lastX;
                mFirstY = lastY;
                int motionPosition = pointToPosition((int)mFirstX, (int)mFirstY);
                int realP = motionPosition;
                boolean isRealEnable = false;
                if(getAdapter() instanceof HeaderViewListAdapter){
                	...
                	realP -= headerCount;
                	if(realP>=0)
                		isRealEnable = headerViewListAdapter.getWrappedAdapter().isEnabled(realP);
                }else{
                	isRealEnable = getAdapter().isEnabled(realP);
                }
                isCurrentChildEnable = realP>=0 && isRealEnable;
                if (isCurrentChildEnable) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dx) >= slop && Math.abs(dy) >= slop) {
                    System.out.println("---->onInterceptTouchEvent——ACTION_MOVE——slop---setLongClickable(false);");
                    return true;
                }
	            ...
	            case MotionEvent.ACTION_MOVE:
	                ...
	                if (mIsHorizontal == null) {
	                    if (!judgeScrollDirection(dx, dy)) {
                            System.out.println("---->line207---setLongClickable(true);");
...
//                            setLongClickable(false);
	                ...
	                if (mIsHorizontal) {
	                    ...
	                    if (dx < 0 && dx > -mRightViewWidth) {
                            ...
                            setLongClickable(false);
                            System.out.println("---->line236---setLongClickable(false);");
	                    }

                        if (dx > 0 && dx > 0.2*mRightViewWidth) {
                            setLongClickable(false);
                            System.out.println("---->向右滑动一定距离了---setLongClickable(false);");
                        }

                        if(Math.abs(dx)>Math.abs(0.1*mRightViewWidth)){
                            setLongClickable(false);
                            System.out.println("---->line248---setLongClickable(false);");
                        }
