...
//	private boolean bAH = false;
	...
	private boolean isScrollToTop = false;//是否滚动到顶�?
	private boolean isScrollFarTop = false;//是否滚动离开了顶�?
//	private boolean isMoveTop = false;//是否向上滑动
	private boolean isMoveDown = false;//是否向下滑动
	private boolean isScrollStoped = false;//滚动是否结束
	private boolean isFristTouch = true;//是否第一次触�?
	...
	public MMPullDownView(Context context, AttributeSet attrs) {
		this(context, attrs,0);
	}

	public MMPullDownView(Context context, AttributeSet attrs, int defStyle) {
...
//		this.adN = this.adO;
	...
	protected void onLayout(boolean changed, int left, int top, int right,
			int bottom) {
		if (!this.bAD){
			View localView2 = inflate(this.context, R.layout.im_pulldown_loading_view, null);
			View localView1 = inflate(this.context, R.layout.im_pulldown_loading_view, null);
